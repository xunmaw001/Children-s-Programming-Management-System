<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.wanzhengshipin" label="完整视频" prop="wanzhengshipin">
          <file-upload
          tip="点击上传完整视频"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.wanzhengshipin?ruleForm.wanzhengshipin:''"
          @change="wanzhengshipinUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.wanzhengshipin" label="完整视频" prop="wanzhengshipin">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.wanzhengshipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengfenlei = true;
          ...
          if(o=='wanzhengshipin'){
            this.ruleForm.wanzhengshipin = obj[o];
	    this.ro.wanzhengshipin = true;
            continue;
          }
          if(o=='fasongshijian'){
	...
	if(this.ruleForm.wanzhengshipin!=null) {
		this.ruleForm.wanzhengshipin = this.ruleForm.wanzhengshipin.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wanzhengkecheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wanzhengkechengCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    wanzhengshipinUploadChange(fileUrls) {
	this.ruleForm.wanzhengshipin = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
