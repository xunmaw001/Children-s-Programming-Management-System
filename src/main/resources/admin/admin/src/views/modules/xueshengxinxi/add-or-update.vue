...
export default {
  ...
  methods: {
    ...
    zhanghaoChange () {
      this.$http({
        url: `follow/jiazhang/zhanghao?columnValue=`+ this.ruleForm.zhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.gerenjianjie = this.ruleForm.gerenjianjie.replace(reg,'../../../springboot760av/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
